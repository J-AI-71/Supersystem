...
(function(){
  ...
  const BOOKMARKLETS = [
    { id:'clean', name:'SafeShare clean', desc:'Aktuelle Seite sauber öffnen (UTM/gclid/fbclid entfernen, Redirects entschnüren).', code: makeCleanCurrent() }
  ];
  ...
  async function copyCode(code, btn){
    ...
    }catch{ prompt('Code kopieren:', code); }
  }
  function renderItem(bm){
    ...
    const btn=el('button',{},'Code kopieren'); btn.onclick=()=>copyCode(bm.code,btn);
    const p=el('p',{class:'mut',style:'margin:4px 0 0'},bm.desc);
    const col=el('div',{style:'flex:1;min-width:280px'}); col.appendChild(input); col.appendChild(p);
    row.appendChild(a); row.appendChild(col); row.appendChild(btn);
    return row;
  }

  window.renderBookmarkletsIndex=function(targetId){
    const root=document.getElementById(targetId); if(!root) return;
    root.innerHTML=''; root.appendChild(renderItem(BOOKMARKLETS[0]));
  };
  window.renderBookmarkletsPage=function(targetId){
    const root=document.getElementById(targetId); if(!root) return;
    root.innerHTML=''; BOOKMARKLETS.forEach(b=>root.appendChild(renderItem(b)));
  };
})();
