...
(function () {
  ...
  // Locale: /en/ oder <html lang="en">
  const path = location.pathname;
  const htmlLang = (document.documentElement.getAttribute("lang") || "").toLowerCase();
  const isEN = path.includes("/en/") || htmlLang.startsWith("en");

  // Links (DE/EN) — passe EN-Pfade an, falls du andere Slugs nutzt
  const LINKS = isEN
    ? {
        home: "/en/",
        app: "/app/en/",
  ...
  // Logo: nutzt dein vorhandenes Icon (kein Emoji)
  const LOGO_SRC = "/assets/fav/favicon.svg";

  const shellHTML = `
<header class="ss-header" role="banner">
  <a class="ss-brand" href="${LINKS.home}" aria-label="SafeShare">
    <img class="ss-brand__mark" src="${LOGO_SRC}" alt="" aria-hidden="true">
...
<div class="ss-moreOverlay" id="ssMoreOverlay" hidden>
  ...
  // Active-State (normalize trailing slash)
  function setActive() {
  ...
  function openMenu() {
    ...
    // Fokus: Close Button (nicht backdrop)
